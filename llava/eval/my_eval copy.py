...
# DataLoader
def create_data_loader(questions,golden_answers, image_folder, tokenizer, image_processor, model_config, batch_size=1, num_workers=1):
...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes, answer), line in tqdm(zip(data_loader, questions), total=len(questions)):
        idx = line["id"]
